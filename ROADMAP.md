...
Current architectural constraints matter to future work: existing profile maps are adapted into map format v4 at startup rather than stored as external files; terrain materials, multi-level collision, reflectors, paired projectile portals, and map-supported projectile boundary rules use typed deterministic data; six versioned procedural player appearances persist locally and synchronize through validated private-room state; React previews and Phaser actors share one normalized character recipe; online team modes use fixed seats and pause everyone for any disconnect; camera modes support variable worlds but do not yet provide manual overview panning; and room lookup is process-local. Match snapshots are version 9 and include map-content identity, selected boundary behavior, player appearances, freeze counters, beacons, and stable next IDs; online compatibility is protocol `private-room-12`, map registry `maps-9`, weapon registry `weapons-4`, appearance registry `appearances-2`, and build `1.10.2`. Existing architecture details remain documented in [README.md](README.md).
...
**Status:** In progress; player appearance customization implemented, control rebinding remains
