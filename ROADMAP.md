...
Current architectural constraints matter to future work: existing profile maps are adapted into map format v3 at startup rather than stored as external files; terrain materials, multi-level collision, reflectors, and paired projectile portals use typed immutable map objects; online team modes use fixed seats and pause everyone for any disconnect; camera modes support variable worlds but do not yet provide manual overview panning; and room lookup is process-local. Match snapshots are version 7 and include map-content identity, freeze counters, beacons, and stable next IDs; online compatibility is protocol `private-room-10`, map registry `maps-8`, weapon registry `weapons-4`, and build `1.8.0`. Existing architecture details remain documented in [README.md](README.md).
