...
- **Map-supported projectile boundaries** let local and private-room players choose open edges, full edge reflection, or horizontal wraparound. The selected rule is server-validated and preserved by snapshots, reconnects, results, and rematches.
...
- **Player customization** provides six locally persisted loadouts with four body silhouettes, validated primary/accent palettes, six patterns, eight faces, and eight accessories. The same procedural identity appears in setup, private lobbies, matches, HUD portraits, turn order, results, reconnects, and rematches; team outlines remain independent from cosmetics.
- **Shared protocol validation** and explicit protocol/snapshot/map/weapon/appearance/build versions live in `src/network/protocol.ts`. The current compatibility contract is protocol `private-room-12`, snapshot `9`, map registry `maps-9`, weapon registry `weapons-4`, appearance registry `appearances-2`, and client build `1.10.2`.
