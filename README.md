...
- **Shared protocol validation** and explicit protocol/snapshot/map/weapon/build versions live in `src/network/protocol.ts`. The current compatibility contract is protocol `private-room-6`, snapshot `6`, weapon registry `weapons-4`, and client build `1.4.1`.
