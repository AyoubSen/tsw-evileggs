...
Reflector walls are authored in world coordinates and snap to the terrain grid. Add them to the metadata `objects` array with a stable lowercase slug ID, `type: "reflector-wall"`, `start` and `end` vectors, `thickness`, and `velocityRetention` from `0.1` through `1`.

Paired bidirectional projectile portals are one atomic object: `{ "id": "portal-pair", "type": "projectile-portal", "entrance": { "start": { "x": 300, "y": 160 }, "end": { "x": 300, "y": 240 }, "thickness": 12 }, "exit": { "start": { "x": 600, "y": 160 }, "end": { "x": 600, "y": 240 }, "thickness": 12 }, "velocityRetention": 0.8 }`. The apertures may not overlap. Each aperture uses the reflector segment bounds and counts separately toward total segment complexity. Maps may contain at most 32 validated atomic objects; IDs are unique across all object types. Use `objects: []` when none are needed.
